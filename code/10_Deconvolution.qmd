...
sample_metadata = sample_metadata |> 
  mutate_at(colnames(deconvolutions$epic), as.numeric)
...
sample_metadata = sample_metadata |> 
  rename(Bcell = `B cell`,
         ...
         NK = `NK cell`,
         CD4 = `T cell CD4+`,
         Other_Cells = `uncharacterized cell`)
...
cell_types = colnames(sample_metadata[,40:47])
...
sample_metadata |>
  dplyr::select(cell_types, last_event, survial_censor) |>
  tidyr::pivot_longer(cols = cell_types, names_to = "Cell_Type", values_to = "Value") |>
...
# #pvalue
# survival_stats$`Bcell`$waldtest[3]
# #hazard ratio
# survival_stats$`Bcell`$conf.int[1]
# #confidence interval lower
# survival_stats$`Bcell`$conf.int[3]
# #confidence interval upper
# survival_stats$`Bcell`$conf.int[4]

survival_stats_df = data.frame(matrix(ncol = 5, nrow = 0))

for(cell_type in cell_types){
  
  pvalue = unname(survival_stats[[cell_type]]$waldtest[3])
  HR = survival_stats[[cell_type]]$conf.int[1]
  lower_interval = survival_stats[[cell_type]]$conf.int[3]
  upper_interval = survival_stats[[cell_type]]$conf.int[4]
...
ggplot(survival_stats_df, aes(x = HR, y = cell_type, xmin = lower_interval, xmax = upper_interval)) +
  geom_point(size = 3) +
