...
plot1 = ggplot(as.data.frame(pca_results$x), aes(x = PC1, y = PC2, label = Row.names, color = tumor_descriptor)) +
    geom_point() +
    geom_label(size = 2) +
...
plot2 = ggplot(as.data.frame(pca_results$x), aes(x = PC1, y = PC2, color = tissue_rollup)) +
    geom_point() +
    scale_color_manual(values = palette_discrete) +
    theme_classic()

#Plot colored by stage
plot3 = ggplot(as.data.frame(pca_results$x), aes(x = PC1, y = PC2, color = ajcc_pathologic_stage)) +
    geom_point() +
    scale_color_manual(values = wes_palette("Zissou1", 13, type = "continuous")) +
    theme_classic()

#png(file = "../outputs/pca_plots.png", height = 350, width = 1400, res = 80)
...
#Appears that some stage I samples are labeled metastatic. Hmmm....
...
#Survival plot
#png(filename = "../outputs/survival_plot_stage.png", height = 600, width = 1200, res = 200)
sample_metadata |> 
